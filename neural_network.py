...
class NetWork(object):
    ...
    def run(self, matrix, iterations):
        """runs network based on the configurations"""
        if self.online_flag:
    ...
    def test(self, element):
        """tests the given element through network with one hidden layer """
    ...
    def gradient_step_batch(self, learning_rate=0.175):
        """gradient step for one iteration.
        In this moment valid for one output layer or one muity layer network with one hidden layer neural network """
        # if network is one out put layer
    ...
    @staticmethod
    def activationSigmoidFunction(layer_weights, input_vector, learning_rate=0.075):

        return 1 / (1 + np.exp(-layer_weights.dot(input_vector) * learning_rate))

    @staticmethod
    def sigmoid_derivative(result_of_act_func):
        return result_of_act_func * (1 - result_of_act_func)  # weight hidden targets
...
if __name__ == '__main__':
    ...
    network.run(matrix_train, 300)
    plt.plot(range(300), network.error_list)
    print(targets_)
    print('{}: [4.8, 3.0, 1.4, 0.3]'.format('setosa'))
